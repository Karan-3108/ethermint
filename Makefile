...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
AMBINET_BINARY = fortressd
AMBINET_DIR = fortress
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
HTTPS_GIT := https://github.com/Karan-3108/fortress.git
DOCKER := $(shell which docker)
DOCKER_BUF := $(DOCKER) run --rm -v $(CURDIR):/workspace --workdir /workspace bufbuild/buf
NAMESPACE := Karan-3108hq
PROJECT := fortress
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=fortress \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(AMBINET_BINARY) \
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/Karan-3108/fortress/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(AMBINET_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(AMBINET_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(AMBINET_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(AMBINET_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/Karan-3108/fortress
.PHONY: format
...
ETHERMINT_URL      	= https://raw.githubusercontent.com/Karan-3108/ethermint/v0.10.0
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(AMBINET_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\fortress\Z fortressd/node "./fortressd testnet --v 4 -o /fortress --keyring-backend=test --ip-addresses fortressdnode0,fortressdnode1,fortressdnode2,fortressdnode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(AMBINET_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/fortress:Z fortressd/node "./fortressd testnet --v 4 -o /fortress --keyring-backend=test --ip-addresses fortressdnode0,fortressdnode1,fortressdnode2,fortressdnode3"; fi
	docker-compose up -d
endif
...
PACKAGE_NAME:=github.com/Karan-3108/fortress
