...
Get started with your Ledger hardware wallet on Ambinet {synopsis}
...
If you want to connect to Ambinet mainnet and Ambinet testnet, you can use the Ethereum Ledger app on Ledger Live by setting the chain ID.
...
|               | EIP155 chain ID |
| ------------- | --------------- |
| Ambinet mainnet | `9001`          |
| Ambinet testnet | `9000`          |
...
::: tip
Follow our [Metamask Guide](./metamask.md) to add the Ambinet Mainnet and Testnet to your Settings
:::
...
### Step-by Cosmos transaction using Ambinet.me
...
- Make sure that the Ethereum Ledger app is installed. The Cosmos Ledger app is not supported on the Ambinet chain at the moment (see [FAQ](#faq)).
...
- If you have Metamask correctly configured, the balance on the extension will be displayed as `AMBINET`, but on the Ledger device it will be displayed as `ETH`.
...
Cosmos `secp256k1` keys are not supported on Ambinet with Ledger. Only Ethereum keys (`eth_secp256k1`) can be generated with Ledger.
...
3. **I can’t generate a key for the Ambinet native multisig using the `fortressd` CLI and and Ledger**
...
Since Ambinet only support Ethereum keys and uses the same HD path as Ethereum, the Cosmos Ledger app doesn’t work to sign cosmos transactions.
