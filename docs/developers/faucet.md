...
Check how to obtain testnet tokens from the Ambinet faucet website {synopsis}

The Ambinet Testnet Faucet distributes small amounts of {{ $themeConfig.project.testnet_denom }} to anyone who can provide a valid testnet address for free. Request funds from the faucet either by using the [Keplr Wallet](./../users/wallets/keplr.md) or follow the instructions on this page.
...
After approval, you can see a transaction confirmation informing you that {{ $themeConfig.project.testnet_denom }} have been successfully transferred to your [fortress address](./../users/technical_concepts/accounts.md#address-formats-for-clients) on the testnet.

::: warning
**Note**: only Ethereum compatible addresses (i.e `eth_secp256k1` keys) are supported on Ambinet.
:::
...
::: warning
If you use your Bech32 address, make sure you input the [account address](./../users/technical_concepts/accounts.md#addresses-and-public-keys) (`fortress1...`) and **NOT** the validator operator address (`fortressvaloper1...`)
:::

![faucet site](./../img/faucet_web_page.png)

View your account balance either by clicking on the Keplr extension or by using the [Testnet Explorer](https://testnet.mintscan.io/fortress-testnet).
