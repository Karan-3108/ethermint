...
The Ambinet supports different clients in order to support Cosmos and Ethereum transactions
and queries:

|                                                        | Description                                                                          | Default Port |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------ | ------------ |
| **Cosmos [gRPC](#cosmos-grpc)**                        | Query or send Ambinet transactions using gRPC                                          | `9090`       |
| **Cosmos REST ([gRPC-Gateway](#cosmos-grpc-gateway))** | Query or send Ambinet transactions using an HTTP RESTful API                           | `9091`       |
...
| **Command Line Interface ([CLI](#cli))**               | Query or send Ambinet transactions using your Terminal or Console.                     | N/A          |

## Cosmos gRPC

Ambinet exposes gRPC endpoints for all the integrated Cosmos SDK modules. This makes it easier for
...
See the list of supported gRPC-Gateway API endpoints for the Ambinet testnet [here](https://api.fortress.dev/).

## Ethereum JSON-RPC

<!-- TODO: Link JSON-RPC docs -->

Ambinet supports most of the standard [JSON-RPC APIs](./json-rpc/server.md) to connect with existing Ethereum-compatible web3 tooling.
...
Check the `Events` page to obtain the event list of each supported module on Ambinet.
:::
...
Users can use the `{{ $themeConfig.project.binary }}` binary to interact directly with an Ambinet node
though the CLI.
