	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				coins := sdk.Coins{sdk.NewCoin("aambinet", sdk.NewInt(100))}
		...
		},
		{
			...
			func() {
				...
				coins := sdk.Coins{sdk.NewCoin("aambinet", sdk.NewInt(2))}
				// update the escrowed account balance to maintain the invariant
				err := testutil.FundModuleAccount(suite.app.BankKeeper, suite.ctx, types.ModuleName, coins)
				suite.Require().NoError(err)
				suite.app.Commit()
		...
		},
		{
			...
			func() {
				...
				coins := sdk.Coins{sdk.NewCoin("aambinet", sdk.NewInt(200))}
				// update the escrowed account balance to maintain the invariant
				err := testutil.FundModuleAccount(suite.app.BankKeeper, suite.ctx, types.ModuleName, coins)
				suite.Require().NoError(err)
				suite.app.Commit()
