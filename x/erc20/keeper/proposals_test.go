...
import (
	...
	"github.com/Karan-3108/ambinet/v4/x/erc20/keeper"
	"github.com/Karan-3108/ambinet/v4/x/erc20/types"
...
func (suite KeeperTestSuite) TestRegisterCoin() {
	...
	testCases := []struct {
		...
		{
			"evm denom registration - ambinet",
			func() {
				metadata.Base = "ambinet"
				err := suite.app.BankKeeper.MintCoins(suite.ctx, inflationtypes.ModuleName, sdk.Coins{sdk.NewInt64Coin(metadata.Base, 1)})
				suite.Require().NoError(err)
			},
			false,
		...
		{
			"evm denom registration - aambinet",
			func() {
				metadata.Base = "aambinet"
				err := suite.app.BankKeeper.MintCoins(suite.ctx, inflationtypes.ModuleName, sdk.Coins{sdk.NewInt64Coin(metadata.Base, 1)})
				suite.Require().NoError(err)
			},
			false,
		...
		{
			"evm denom registration - wambinet",
			func() {
				metadata.Base = "wambinet"
				err := suite.app.BankKeeper.MintCoins(suite.ctx, inflationtypes.ModuleName, sdk.Coins{sdk.NewInt64Coin(metadata.Base, 1)})
				suite.Require().NoError(err)
			},
			false,
