...
import (
	...
	"github.com/Karan-3108/ambinet/v4/ibc"
	ambinet "github.com/Karan-3108/ambinet/v4/types"
...
//  - sends over all Ambinet native tokens
...
func (k Keeper) OnRecvPacket(
	...
	// Get addresses in `ambinet1` and the original bech32 format
	...
	// a sender's own account on Ambinet (sender == recipient)
	...
	if account != nil && ambinet.IsSupportedKey(account.GetPubKey()) {
		return ack
	}
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		err = k.transferKeeper.SendTransfer(
			...
			recipient,                 // recipient is the address in the Ambinet chain
...
// the IBC denomination, i.e port and channel on Ambinet for the voucher. It
// returns an error if:
