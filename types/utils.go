...
const (
	// MainnetChainID defines the Ambinet EIP155 chain ID for mainnet
	MainnetChainID = "ambinet_9001"
	// TestnetChainID defines the Ambinet EIP155 chain ID for testnet
	TestnetChainID = "ambinet_9000"
)

// IsMainnet returns true if the chain-id has the Ambinet mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Ambinet testnet EIP155 chain prefix.
...
// GetAmbinetAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `ambinet`).
// The function fails if the provided bech32 address is invalid.
func GetAmbinetAddressFromBech32(address string) (sdk.AccAddress, error) {
